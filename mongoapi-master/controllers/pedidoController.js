...
const Pedido = require("../models/User");
...
exports.recibirPedido = async (req, res) => {
  try {
    ...
    if (pedido.estado === "Creado") {
      ...
      const cliente = await Pedido.findById(pedido.usuario);
      if (cliente) {
...
exports.obtenerPedidoClientePorId = async (req, res) => {
  try {
...
exports.obtenerPedidosCliente = async (req, res) => {
  try {
    const pedido = await Pedido.find({ activo: true }).populate(
      "nombre"
    );
    res.json(pedido);
  } catch (error) {
...
exports.cancelarPedido = async (req, res) => {
  try {
    ...
    if (
      ...
      pedido._id.toString() !== req.user.userId
    ) {
      return res
        .status(403)
